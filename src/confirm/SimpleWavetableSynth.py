...
class SimpleWavetableSynth(EvalModelInit):
    ...
    def _save_wav(self, audio:np.ndarray or torch.tensor, attrs:dict, frequency:int, addFilename:str=""):
...
if __name__ == "__main__":
    ...
    eval = False
    x, attrs = simpleWavetableSynth.read_waveform(idx=0,latent_op=None,eval=eval,plot=False,save=False,title="Input : ")
    ...
    eval = True
    x, attrs = simpleWavetableSynth.read_waveform(idx=0,latent_op=None,eval=eval,plot=False,save=False,title="Regenerate : ")
    simpleWavetableSynth.wavetableSynth(
        wavetable=x,
        sample_rate=int(attrs["samplerate"]),
        duration=duration,
